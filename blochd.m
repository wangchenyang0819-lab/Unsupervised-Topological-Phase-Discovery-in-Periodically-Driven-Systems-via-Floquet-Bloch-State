...
for i = 1:m
    A(1, i) = 1*pi/T;      % Coupling coefficient 1
    A(2, i) = -2*pi/T;     % Coupling coefficient 2
end
...
for i = 1:m
    for k = 1:N
        for t = 1:N
            bohanshu = [bohanshut1(k, t, i); bohanshut2(k, t, i)];
...
for k = 1:(N-1)/2
    for t = 1:(N-1)
        ...
        beili = 1/(4*pi) * dot(vec1, cross(vec2, vec3)) / (2*pi/N) / (T/N);
        chern = chern + beili * (2*pi/N) * (T/N);
    end
end
